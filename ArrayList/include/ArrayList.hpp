...
class ArrayList{
	...
	public:
		...
		int indexOf(const Object& item) const throw(ElemanYok)
		{
		...
		const Object& elementAt(int i) const throw(ElemanYok)
		{
			if(i<0 || i>=length) throw ElemanYok("item bulunamadi"); 
			return items[i]; 
		}
		void remove(const Object& item) throw(ElemanYok)
		{
			for(int i=0;i<length;i++){
				if(items[i] == item){
					removeAt(i);
		...
		void insert(int i,const Object& yeni) throw(Tasma)
		{
